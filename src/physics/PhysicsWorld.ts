...
export class PhysicsWorld {
  ...
  static standardRack(): PhysicsWorld {
    ...
    for (let row = 0; row < 5; row += 1) {
      for (let i = 0; i <= row; i += 1) {
        w.addBall(new BallBody(id++, (i - row / 2) * spacing, -0.42 - row * spacing * 0.8660254));
      }
    }
    return w;
  }
  ...
  stepFrame(frameDt: number): void {
    this.accumulator += Math.min(0.1, Math.max(0, frameDt));
  ...
  fixedStep(dt = PHYSICS.FIXED_DT): void {
    ...
    // Deterministic microstepping is continuous-collision protection inside each 120 Hz tick.
  ...
  strikeCue(aimAngle: number, power: number, contactX = 0, contactY = 0): ShotRecord {
    ...
    const preShot = this.balls.map((b) => b.snapshot());
    const cuePosition = { ...cue.position };
    const p = Math.max(0, Math.min(1, power));
    const dx = Math.sin(aimAngle), dz = -Math.cos(aimAngle);
    const impulseMagnitude = PHYSICS.CUE_MAX_IMPULSE * p;
    const vx = dx * impulseMagnitude / cue.mass, vz = dz * impulseMagnitude / cue.mass;
    const speed = Math.hypot(vx, vz);

    this.events = [];
    this.shotStartTime = this.time;
    const record: ShotRecord = {
      preShotBallStates: preShot,
      cueBallPosition: cuePosition,
      aimAngle,
      shotPower: p,
  ...
  finishShotIfSettled(): ShotRecord | null {
    ...
    this.activeShot.simulationDuration = this.time - this.shotStartTime;
  ...
  isMoving(): boolean {
    return this.balls.some((b) => !b.pocketed && !b.sleeping && (
      Math.hypot(b.velocity.x, b.velocity.z) > PHYSICS.SLEEP_LINEAR ||
      Math.hypot(b.angularVelocity.x, b.angularVelocity.y, b.angularVelocity.z) > PHYSICS.SLEEP_ANGULAR
    ));
  }
  ...
  private handleBallCollisions(): void {
    ...
    for (let i = 0; i < this.balls.length; i += 1) {
      ...
      for (let j = i + 1; j < this.balls.length; j += 1) {
        ...
        const nx = dist > EPS ? dx / dist : 1, nz = dist > EPS ? dz / dist : 0;
        const correction = Math.max(0, diameter - dist - PHYSICS.POSITION_SLOP) * PHYSICS.POSITION_CORRECTION * 0.5;
        ...
        const normalImpulse = -(1 + PHYSICS.BALL_RESTITUTION) * vn / invMass;
        const ix = normalImpulse * nx, iz = normalImpulse * nz;
        a.velocity.x -= ix / a.mass; a.velocity.z -= iz / a.mass;
        b.velocity.x += ix / b.mass; b.velocity.z += iz / b.mass;

        const tx = -nz, tz = nx;
        const vt = rvx * tx + rvz * tz;
        const tangentImpulse = Math.max(-normalImpulse * PHYSICS.BALL_FRICTION, Math.min(normalImpulse * PHYSICS.BALL_FRICTION, -vt / invMass));
        a.velocity.x -= tangentImpulse * tx / a.mass; a.velocity.z -= tangentImpulse * tz / a.mass;
        b.velocity.x += tangentImpulse * tx / b.mass; b.velocity.z += tangentImpulse * tz / b.mass;
        const spinImpulse = (2.5 * tangentImpulse) / (a.mass * a.radius);
        a.angularVelocity.y -= spinImpulse; b.angularVelocity.y += spinImpulse;
        a.wake(); b.wake();
        this.emit({ type: 'ball', time: this.time, a: a.id, b: b.id, impulse: Math.abs(normalImpulse) });
      }
    }
  }
  ...
  private railAxis(b: BallBody, axis: 'x' | 'z', limit: number, rail: 'left' | 'right' | 'top' | 'bottom', nx: number, nz: number): void {
    ...
    if (!beyond || this.nearPocketOpening(b.position.x, b.position.z, axis)) return;
    ...
    const maxTangent = Math.abs(deltaN) * PHYSICS.RAIL_TANGENTIAL_FRICTION;
    const tangentDelta = Math.max(-maxTangent, Math.min(maxTangent, -(vt - sideSurface)));
  ...
  private nearPocketOpening(x: number, z: number, axis: 'x' | 'z'): boolean {
    ...
    for (const b of this.balls) {
      ...
      for (let i = 0; i < pockets.length; i += 1) {
        ...
        if (deep || (speed < PHYSICS.POCKET_CAPTURE_SPEED && toward > -0.15)) {
          ...
          b.sleeping = true;
          if (b.id === 0) this.emit({ type: 'scratch', time: this.time, ball: b.id });
          else this.emit({ type: 'pocket', time: this.time, ball: b.id, pocket: i });
          break;
        }
  ...
  private emit(event: SimulationEvent): void {
    ...
    if (!shot) return;
    if (event.type === 'ball' && shot.firstObjectBallContact == null) {
      if (event.a === 0) shot.firstObjectBallContact = event.b;
      else if (event.b === 0) shot.firstObjectBallContact = event.a;
    }
    if (event.type === 'rail') shot.railContacts.push({ ball: event.ball, rail: event.rail, time: event.time });
    if (event.type === 'pocket' && !shot.ballsPocketed.includes(event.ball)) shot.ballsPocketed.push(event.ball);
    if (event.type === 'scratch') shot.cueBallScratch = true;
  }
  ...
  private assertFinite(): void {
    for (const b of this.balls) {
      ...
      if (speed > PHYSICS.MAX_SPEED) {
        ...
        b.velocity.x *= scale; b.velocity.z *= scale;
      }
    }
  }
}
